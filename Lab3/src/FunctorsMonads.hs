...
class Applicative' m => Monad' m where
  (>>==) :: m a -> (a -> m b) -> m b
  monadObj >>== f = join' (f <$$> monadObj)
  join' :: m (m a) -> m a
  join' monadObj = monadObj >>== id
...
liftA2' f appl1 appl2 = f <$$> appl1 <**> appl2
...
filterA p [] = pure' []
filterA p (x:xs) = let
    cond b rest = if b then x:rest else rest
  in cond <$$> p x <**> filterA p xs
